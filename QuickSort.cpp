...
template <class H>
void quickSort<H>::quick(int top, int bottom) {
  int middle;
  if (top < bottom) {
...
template <class H>
int quickSort<H>::partition(int top, int bottom) {
  H x = array[top];
  int i = top - 1;
  int j = bottom + 1;
  do {
...
template <class H>
void quickSort<H>::swap(int i, int j) {
  H tmp = array[i];
  array[i] = array[j];
  array[j] = tmp;
}
...
template <class H>
H quickSort<H>::addElement(int x) {
  H temp;
